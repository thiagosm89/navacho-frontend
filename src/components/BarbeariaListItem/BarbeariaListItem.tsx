...
import ResumoIA from '../ResumoIA/ResumoIA'
import ModalAvaliacoes from '../ModalAvaliacoes/ModalAvaliacoes'
import './BarbeariaListItem.css'
...
interface BarbeariaListItemProps {
  barbearia: Barbearia
}

const BarbeariaListItem = ({ barbearia }: BarbeariaListItemProps) => {
  const [mostrarAvaliacoes, setMostrarAvaliacoes] = useState(false)
  const [mostrarModal, setMostrarModal] = useState(false)

  // Mostrar apenas as 5 últimas avaliações
  const avaliacoesRecentes = barbearia.avaliacoes.slice(-5).reverse()
  const temMaisAvaliacoes = barbearia.avaliacoes.length > 5
  ...
  return (
    <div className="barbearia-list-item">
      <div className="list-item-main">
        <div className="list-item-header">
          <div className="list-item-header-top">
            <h2 className="list-item-nome">{barbearia.nome}</h2>
            {barbearia.distancia !== undefined && (
              <span className="list-item-distancia">
          ...
          <div className="list-item-avaliacao">
        ...
        <div className="list-item-servicos">
      ...
      {mostrarAvaliacoes && (
        <div className="list-item-avaliacoes">
          <h3 className="avaliacoes-title">Avaliações dos Clientes</h3>
          
          <ResumoIA avaliacoes={barbearia.avaliacoes} />

          <div className="avaliacoes-recentes">
            <h4 className="avaliacoes-subtitle">Últimas Avaliações</h4>
          ...
          {temMaisAvaliacoes && (
            <button 
              className="btn-ver-mais-avaliacoes"
              onClick={() => setMostrarModal(true)}
            >
              Ver Todas as Avaliações ({barbearia.avaliacoes.length})
            </button>
          )}
        </div>
      )}
      ...
      {mostrarModal && (
        <ModalAvaliacoes
          ...
          totalAvaliacoes={barbearia.totalAvaliacoes}
...
export default BarbeariaListItem
